...
import { Volume2, VolumeX, ArrowDown, ExternalLink, Terminal, Brain, Code2, Sparkles, X, Zap } from 'lucide-react';
import './style.css';
import './fun.css';
import './reaction.css';
...
const memeSounds = [
  { name: 'Spider-Man Meme Song', url: 'https://www.myinstants.com/media/sounds/spiderman-meme-song.mp3' },
...
function App() {
  ...
  const playIntroSound = () => {
    try {
      ...
      const player = new Audio(introSound.url);
      player.preload = 'auto';
      player.volume = 0.18;
      audioElement.current = player;
      player.play().catch(() => {});
  ...
  const react = (text, forcedSound) => {
    ...
    if (forcedSound) {
      ...
      if (sound && (interactionCount.current % 3 === 0 || Math.random() < 0.22)) playMemeSound();
      else beep();
    }
  ...
  useEffect(() => {
    // Try immediately on page load. If autoplay is blocked, the first user
    // interaction unlocks the same sound without requiring the sound toggle.
    playIntroSound();
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (!document.hidden && Math.random() < 0.32) playMemeSound();
    }, 22000);
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {toast && <motion.div className="toast surprise-toast" initial={{ y: -40, opacity: 0, scale: 0.9, rotate: -3 }} animate={{ y: 0, opacity: 1, scale: 1, rotate: 0 }} exit={{ y: -40, opacity: 0 }}>{toast}</motion.div>}
      </AnimatePresence>

      <AnimatePresence>
        {modal && (
          <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModal(null)}>
            <motion.div className="modal" initial={{ scale: 0.8, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.8, y: 30 }} onClick={(event) => event.stopPropagation()}>
      ...
      {intro && (
        <motion.div className="intro" initial={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.03 }} transition={{ duration: 0.8 }}>
          ...
          <div className="intro-copy">
            ...
            <button onClick={() => { playIntroSound(); setIntro(false); react('Ready aa? Let’s go!', 'Anime Wow'); }}>ENTER THE MASS <ArrowDown size={18} /></button>
      ...
      <header>
        ...
        <button className="sound" onClick={() => setSound((value) => !value)}>{sound ? <Volume2 /> : <VolumeX />}<span>{sound ? 'SOUND ON' : 'SOUND OFF'}</span></button>
      </header>
      ...
      <main id="home" className={chaos ? 'chaos' : ''}>
        <section className="hero">
          ...
          <div className="hero-left">
            ...
            <div className="hero-actions"><a href="#projects" onClick={() => react('Idhi vere level!')}>ENTER THE PROJECTS <ArrowDown size={18} /></a><a href="mailto:krishnasathwik23@gmail.com" className="ghost" onClick={() => react('Sare, mail pampu!')}>LET'S TALK</a></div>
            <div className="hero-meta"><span><i /> AVAILABLE FOR COOL BUILDS</span><span>INDIA · 2026</span></div>
          </div>
          <div className="portrait-wrap"><div className="portrait-glow" /><div className="portrait-ring" /><img src="https://avatars.githubusercontent.com/u/217095135?v=4" className="portrait" alt="Krishna Sathwik" /><div className="portrait-frame">PORTRAIT_001<br /><b>MAIN CHARACTER</b></div><div className="stamp">AI<br />×<br />CODE</div></div>
        ...
        <section id="skills" className="section skills">
          ...
          <div className="skill-cloud">{skills.map((skill, index) => <motion.button className="skill" key={skill} whileHover={{ scale: 1.1, rotate: index % 2 ? 2 : -2 }} onClick={() => react(`${skill} — super!`)}><span>{String(index + 1).padStart(2, '0')}</span>{skill}</motion.button>)}</div>
        ...
        <section id="projects" className="section projects">
          ...
          {projects.map((project, index) => { const Icon = project.icon; const projectSound = index === 0 ? 'Vine Boom' : index === 1 ? 'Dun Dun Dun' : 'Anime Wow'; const projectReaction = index === 0 ? 'Abbaa! LearnShot!' : index === 1 ? 'Ohooo! Decision time!' : 'Ayyayyo! Health AI!'; return (
            <motion.article className="project" key={project.title} initial={{ opacity: 0, y: 70 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.7 }} onClick={() => { setModal(project); react(projectReaction, projectSound); }}>
              <div className="project-no">{project.n}</div><div className="project-main"><div className="project-top"><span>AI / BUILD</span><span>CLICK FOR INTEL ↗</span></div><h3>{project.title}</h3><p>{project.desc}</p><div className="stack">{project.stack.map((item) => <span key={item}>{item}</span>)}</div><div className="project-action">OPEN PROJECT INTEL <Zap size={15} /></div></div>
        ...
        <section id="contact" className="contact">
          <div className="contact-no">04</div><p>INKA MATLADUKUNDAAMA?</p><h2>LET'S MAKE<br /><em>SOMETHING CRAZY.</em></h2>
          <a href="mailto:krishnasathwik23@gmail.com" onClick={() => react('Sare, matladukundham!')}>krishnasathwik23@gmail.com</a>
          <div className="socials"><a href="https://github.com/KrishnaSathwik23" target="_blank" rel="noreferrer" onClick={() => react('GitHub ki vellam!')}>⌘ GITHUB</a><a href="https://linkedin.com/in/krishnasathwikmandavilli" target="_blank" rel="noreferrer" onClick={() => react('LinkedIn time!')}>in LINKEDIN</a></div>
        </section>
      </main>

      <footer><span>KRISHNA SATHWIK © 2026</span><span>MADE WITH CODE, CHAOS & TELUGU</span><button onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); react('Back to top!'); }}>BACK TO TOP ↑</button></footer>
    </>
  );
}
